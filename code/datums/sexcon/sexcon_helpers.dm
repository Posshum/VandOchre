...
/mob/living/carbon/human/MiddleMouseDrop_T(mob/living/target, mob/living/user)
	...
	if(!user.can_do_sex())
		...
		return
	//Make a check to remove character honor here, and to allow the target to amend the failed attempt in case it was an accident.
	if(!target.client || !target.client.prefs || (target.client.prefs.sexable == FALSE)) // Don't bang someone that doesn't want it.
		if(target == user)
			to_chat(target, "<span class='warning'>You clutch your groin. (Your ERP Preferences is under Options.)</span>")
			return
		//adjust_character_honor(CH_LOSE_SEXACT, user, reason = "Attempted unconsentual sex actions against someones prefs.")
		to_chat(user, "<span class='warning'>[target] doesn't wish to be touched. Your honor dropped slightly.</span>")
		to_chat(target, "<span class='warning'>[user] failed to touch you. (Your ERP Preferences is under Options.)</span>")
		return
