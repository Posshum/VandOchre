...
/mob/living/proc/add_splatter_wall(mob/M, turf/T, force, spill_amount)
	...
	if(abs(force) <= 0) //If the force doesn't do enough damage then dont do anything.
		return
	if(!iscarbon(src))
		if(!HAS_TRAIT(src, TRAIT_SIMPLE_WOUNDS))
			return
	var/datum/blood_type/blood = get_blood_type()
	if(!blood.reagent_type)
		return
	if(!T)
		T = get_turf(src)
	for(var/turf/closed/w in orange(abs(force_distance), T))
		var/loc = get_step(T, M)
		var/obj/effect/decal/cleanable/blood/splatter/walls/splatter = new /obj/effect/decal/cleanable/blood/splatter/walls(loc)
		splatter.transfer_mob_blood_dna(src)
		splatter.update_appearance()
		T?.pollute_turf(/datum/pollutant/metallic_scent, 30)
		if(spill_amount > 0)
